...
def distorted_cropped_image(pil_image, image_size, quality):
    cropped_image = center_crop_arr(pil_image, image_size)
    buffer = io.BytesIO()
    cropped_image.save(buffer, 'JPEG', quality=quality)
    buffer.seek(0)
    cropped_image = Image.open(buffer)
...
def main(args):
    ...
    transform = transforms.Compose([
        transforms.Lambda(lambda pil_image: distorted_cropped_image(pil_image, args.image_size, args.image_quality)),
    ...
    feats = np.zeros([loader_len, 4, 32, 32], dtype=np.float32)
    labels = np.zeros([loader_len,],dtype=np.int64)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, default='/home/ubuntu/data/datasets/imangenet1k')
